...
def check_python_install(source_root: Path, workdir: Path) -> None:
    ...
    run(
        [
            str(python),
            "-c",
            "from pathlib import Path; from runcost import aggregate_cost_ledgers, calculate_cost, from_response, extract_openai_embeddings_usage, from_ag2_usage_summary, from_haystack_generator_result, from_litellm_response, track_langchain_costs, price_cards_from_helicone, price_cards_from_json_file, price_cards_from_yaml_file, price_cards_from_models_dev, price_cards_from_official_snapshot, price_cards_from_source_cache, price_cards_from_user_pricing; p=Path('prices.json'); p.write_text('{\"provider\":\"test\",\"models\":[{\"id\":\"test\",\"prices\":{\"input\":\"1\"}}]}'); y=Path('prices.yaml'); y.write_text('provider: test\\nmodels:\\n  - id: test\\n    prices:\\n      input: \"1\"\\n'); print(aggregate_cost_ledgers, calculate_cost, from_response, extract_openai_embeddings_usage, from_ag2_usage_summary, from_haystack_generator_result, from_litellm_response, track_langchain_costs, price_cards_from_helicone, price_cards_from_json_file(p), price_cards_from_yaml_file(y), price_cards_from_models_dev, price_cards_from_official_snapshot, price_cards_from_source_cache, price_cards_from_user_pricing)",
        ],
        workdir,
    )
