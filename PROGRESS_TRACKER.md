...
Current active lane: Milestone 3 source adapter hardening, focused on source-cache, historical pricing, local JSON price files, explicit source refresh, models.dev enrichment, reviewed official pricing snapshots, and source capability warnings.
...
- `npm test` passes.
- Python and JavaScript fixture runner checks 67 shared fixtures, with fixture metadata allowing language-scoped framework ergonomics fixtures.
...
| Add fixture metadata fields | Done | `schemas/fixture.schema.json`; all 67 fixtures include `metadata` | Metadata covers requirement IDs, provider, surface, scenario, tags, and expected languages. |
...
| Milestone 3: Source Adapter Layer | Partial | Yes | `llm-prices` current and historical feeds, LiteLLM, Portkey, OpenRouter models, models.dev, reviewed official snapshots, source-cache envelopes, local JSON file loading, explicit refresh command, source capability warnings, user compact pricing, and Helicone prototype adapters exist. | YAML file loading. |
...
- `python3 scripts/check_fixtures.py` passed with 67 fixtures across declared Python and JavaScript fixture coverage after the source capability warning slice.
- `python3 scripts/check_fixture_coverage.py` passed with metadata on all 67 fixtures and a current checked-in coverage report after the source capability warning slice.
...
3. Source adapter completeness: YAML file loading.
