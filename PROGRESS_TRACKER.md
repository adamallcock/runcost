...
Current active lane: Milestone 3 source adapter hardening, focused on offline source-cache semantics.
...
- `npm test` passes.
- Python and JavaScript fixture runner checks 62 shared fixtures, with fixture metadata allowing language-scoped framework ergonomics fixtures.
...
| Add fixture metadata fields | Done | `schemas/fixture.schema.json`; all 62 fixtures include `metadata` | Metadata covers requirement IDs, provider, surface, scenario, tags, and expected languages. |
...
| Milestone 3: Source Adapter Layer | Partial | Yes | `llm-prices`, LiteLLM, Portkey, OpenRouter models, source-cache envelopes, user compact pricing, and Helicone prototype adapters exist. | Official pricing snapshots, models.dev enrichment, file-based YAML/JSON loaders, explicit refresh command, source capability warnings, and historical feed semantics. |
...
- `python3 scripts/check_fixtures.py` passed with 62 fixtures across declared Python and JavaScript fixture coverage after the source-cache slice.
- `python3 scripts/check_fixture_coverage.py` passed with metadata on all 62 fixtures and a current checked-in coverage report after the source-cache slice.
...
- Source adapters for `llm-prices`, LiteLLM, Portkey, OpenRouter models, source-cache envelopes, user compact pricing, and Helicone model-registry data.
...
- Source adapters: core adapters exist for seven sources, including source-cache envelopes, user compact pricing, and Helicone model-registry data. Provider official pricing snapshots, models.dev enrichment, full file-reading YAML loaders, explicit refresh command, source capability warnings, and historical feed semantics are still missing.
...
3. Source adapter completeness: official snapshots, full file-reading YAML loaders, explicit refresh command, source capability warnings, and historical feed semantics.
...
4. Source adapter hardening slice: add official snapshots, full file-reading YAML loaders, explicit refresh command, source capability warnings, and historical feed semantics.
