...
Current active lane: Milestone 3 source adapter hardening, focused on source-cache, historical pricing, local JSON price files, explicit source refresh, models.dev enrichment, and reviewed official pricing snapshots.
...
- `npm test` passes.
- Python and JavaScript fixture runner checks 66 shared fixtures, with fixture metadata allowing language-scoped framework ergonomics fixtures.
...
| Add fixture metadata fields | Done | `schemas/fixture.schema.json`; all 66 fixtures include `metadata` | Metadata covers requirement IDs, provider, surface, scenario, tags, and expected languages. |
...
| Milestone 3: Source Adapter Layer | Partial | Yes | `llm-prices` current and historical feeds, LiteLLM, Portkey, OpenRouter models, models.dev, reviewed official snapshots, source-cache envelopes, local JSON file loading, explicit refresh command, user compact pricing, and Helicone prototype adapters exist. | YAML file loading and source capability warnings. |
...
- `python3 scripts/check_fixtures.py` passed with 66 fixtures across declared Python and JavaScript fixture coverage after the reviewed official snapshot adapter slice.
- `python3 scripts/check_fixture_coverage.py` passed with metadata on all 66 fixtures and a current checked-in coverage report after the reviewed official snapshot adapter slice.
...
- Source adapters: core adapters exist for nine source families plus local JSON file loading and an explicit source refresh command, including source-cache envelopes, models.dev enrichment, reviewed official pricing snapshots, user compact pricing, and Helicone model-registry data. YAML file loading and source capability warnings are still missing.
...
3. Source adapter completeness: YAML file loading and source capability warnings.
