...
Registry-side trusted publishing verification is tracked separately from the
workflow code. The checked-in template lives at
`fixtures/source-files/trusted-publishing-verification-template.json` and
follows `schemas/trusted-publishing-verification.schema.json`.

Before public beta, replace the template with a sanitized manual verification
artifact or pass a separate reviewed artifact to:

```bash
npm run check:trusted-publishing -- \
  --verification /tmp/runcost-trusted-publishing-verification.json \
  --require-real
```

Real verification must use `evidence_type: "manual_verified"`, confirm both
PyPI and npm trusted publishers, confirm OIDC/provenance configuration, and
confirm no stored registry token is required. Do not include registry tokens,
API keys, screenshots with account identifiers, or private registry settings.
The verification must identify PyPI package `runcost-ai` and npm package
`runcost`.
...
- Keep publishing disabled until PyPI/npm trusted publishers are configured,
  artifacts are reviewed, and the explicit typed publish approval is provided.
...
Current release evidence for `0.2.0`, including GitHub Release, PyPI, npm, Go,
...
`runcost`. The naming decision is recorded in
...
matching PyPI distribution during the latest check, and npm `runcost` returned
`E404 Not Found`. Configure the PyPI trusted publisher for `runcost-ai`, not
the occupied `runcost` project.
