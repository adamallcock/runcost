...
Runnable dependency-free examples:

- `examples/python_framework_adapters.py` shows OpenAI Agents SDK and LangSmith
  usage-shaped objects with provider-reported cost comparison warnings.
- `examples/javascript_framework_adapters.mjs` shows Vercel `streamText`
  finish/onFinish handling and OpenRouter SDK response cost comparison.
