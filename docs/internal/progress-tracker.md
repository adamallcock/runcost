...
Keep `docs/internal/project-plan.md` implementation state explicit while closing current-scope roadmap gaps, preserving the polyglot, schema-first, fixture-first architecture across Python, JavaScript/TypeScript, Go, and future languages.
...
- `Partial`: meaningful fixture-backed work exists, but the milestone exit gate in `docs/internal/project-plan.md` is not yet satisfied.
...
- Fixture metadata and checked-in coverage report pass through `python3 scripts/check_fixture_coverage.py` with 106 fixtures.
...
- Alpha smoke evidence contract checks pass through `python3 scripts/check_alpha_smoke_contract.py`; `schemas/alpha-smoke-report.schema.json` documents sanitized smoke and preflight report shape with mode-specific rules for aggregate smoke, single framework reports, and preflight reports, and the checker rejects forbidden keys, secret-like values, raw responses, missing sample-price acknowledgement, summary drift, and schema/checker drift. The aggregate, LangChain, and Vercel smoke runners now validate sanitized report shape before writing output, and aggregate framework child-script failures become sanitized `needs_product_truth` findings.
- Alpha smoke preflight passes through `python3 scripts/check_alpha_smoke_preflight.py`; the current environment reports OpenAI, Anthropic, OpenRouter, OpenAI Costs invoice comparison, Vercel AI SDK, and LangChain live scenarios as not ready because credentials, required comparison inputs, and/or optional framework packages are missing, while the local multi-provider discount scenario is ready. Preflight scenarios include sanitized `next_action` guidance using environment variable names, command-input names, and dependency names only.
- Alpha smoke product-truth classification checks pass through `python3 scripts/check_alpha_product_truth.py`; `schemas/alpha-smoke-product-truth-register.schema.json` documents the register shape, and the current no-credential live report is tied to machine-readable documented-limitation entries in `fixtures/source-files/alpha-smoke-product-truth-register.json`. The checker now rejects non-passing entries with `classification: none` and mismatched classification/artifact-kind pairs.
- Alpha smoke product-truth entry generation checks pass through `python3 scripts/create_alpha_product_truth_entry.py --self-check`; future sanitized smoke findings can be converted into schema-valid register entries without hand-authoring scenario/status/next-action fields.
- Invoice/dashboard comparison sample checks pass through `python3 scripts/check_invoice_comparison.py`; comparison outputs include `evidence_type` and `milestone8_real_evidence` so the checked-in sample cannot be mistaken for real provider dashboard/export evidence. The comparison producer now rejects unsafe/private/secret-like input before writing output, and the OpenAI Costs API comparison-input converter plus sample/live runner are covered by the same checker.
- Invoice/dashboard comparison contract checks pass through `python3 scripts/check_invoice_comparison_contract.py`; `schemas/invoice-dashboard-comparison-input.schema.json` documents sanitized provider-export reduction input shape, `schemas/invoice-dashboard-comparison.schema.json` documents sanitized comparison output shape, and the checker rejects invalid sanitized inputs, forbidden keys, secret-like values, private export flags, summary drift, and discrepancies without product-truth actions.
...
- Alpha evidence bundle validation passes through `python3 scripts/check_alpha_evidence_bundle.py`; the future strict form requires all Milestone 8 smoke scenarios, product-truth classifications, sample-price acknowledgement, and the real invoice/dashboard comparison in one reviewed evidence set.
- Alpha evidence bundle collection passes through `python3 scripts/check_alpha_evidence_collector.py`; `npm run alpha:bundle -- --mode sample --output-dir /tmp/runcost-alpha-evidence-bundle --allow-sample-prices` creates a sanitized manifest plus preflight, aggregate smoke, Vercel, LangChain, and invoice comparison artifacts, then validates the bundle.
- Go package passes `go test ./packages/go/...`.
- Explicit Go fixture conformance passes through `python3 scripts/check_go_fixtures.py`, making the Go fixture lane visible alongside the Python and JavaScript fixture runner.
...
- Python compile check passes for package, scripts, and Python examples.
- Python public type-surface checks pass through `python3 scripts/check_python_type_surface.py`; public Python runtime APIs listed in the API registry must retain parameter and return annotations.
- JavaScript and Python provider and framework examples run.
...
- Release readiness now verifies the release workflow uses OIDC/provenance publishing shape and does not depend on stored PyPI/npm registry tokens.
- Release readiness now verifies the guarded release workflow requires typed `publish_approval=publish-runcost` before any `publish=true` dispatch can proceed.
- Trusted-publishing verification checks pass through `python3 scripts/check_trusted_publishing_verification.py`; the checked-in artifact is an explicit template for PyPI `runcost-ai` and npm `runcost`, and strict `--require-real` mode remains pending until registry-side settings are manually verified.
- Package-name availability check exists at `docs/internal/reports/2026-05-26-package-name-availability-check.md`; npm `runcost` returned `E404 Not Found`, while PyPI `runcost` is already occupied.
- Python distribution-name decision exists at `docs/internal/decisions/2026-05-27-python-distribution-name.md`; live registry checks on 2026-05-27 confirmed PyPI `runcost` is occupied, PyPI `runcost-ai` has no matching distribution, and the repo now uses Python distribution name `runcost-ai` while preserving `import runcost` and the `runcost` CLI.
...
- Guarded GitHub release workflow passed from `main` with
  ...
  `docs/internal/reports/2026-05-26-release-workflow-no-publish-rehearsal.md`.
...
- Real Go module tag verification passed for remote tag `v0.1.0` in guarded
...
- Project plan exists in `docs/internal/project-plan.md`.
- Polyglot decision record exists in `docs/internal/decisions/polyglot-toolchain-decision.md`.
- Public API parity matrix exists in `docs/internal/notes/api-parity-matrix.md`.
- Machine-readable public API registry exists in `fixtures/source-files/public-api-registry.json`, with generated docs at `docs/generated/public-api-registry.md` and drift/export checks in `scripts/check_public_api_registry.py`.
- Generated public beta/V1 caveats exist in `docs/generated/beta-v1-caveats.md` and are drift-checked from the project completion gate register.
- The project completion gate register has a schema-backed contract in `schemas/project-completion-gates.schema.json`, and `scripts/check_project_completion_gates.py` now checks schema/validator parity for allowed statuses, areas, requirements, evidence kinds, required gate fields, and status-dependent gate rules.
- V1 stabilization checklist checks pass through `python3 scripts/check_v1_stabilization_checklist.py`; the checked-in checklist is an explicit template, and strict `--require-real` mode remains pending until a release-candidate review can prove every readiness field.
...
- Registry README policy exists in `docs/internal/process/release-process.md`; PyPI uses the root README and npm carries `packages/javascript/core/README.md`.
...
Source: `docs/internal/project-plan.md`, section "Immediate Next Sprint".
...
| Add polyglot toolchain decision record | Done | `docs/internal/decisions/polyglot-toolchain-decision.md`; hygiene check passes | Decides JSON Schema plus shared fixtures as v0.x source of truth, with typed artifacts now and generation later. |
...
| Add public API parity matrix | Done | `docs/internal/notes/api-parity-matrix.md`; hygiene check validates public API names | Tracks Python, JS/TS, and Go support by capability. |
...
| Add fixture metadata fields | Done | `schemas/fixture.schema.json`; all 89 fixtures include `metadata` | Metadata covers requirement IDs, provider, surface, scenario, tags, and expected languages. |
| Add fixture coverage report | Done | `docs/internal/reports/fixture-coverage.md`; `scripts/check_fixture_coverage.py`; `npm test` passes | Reports scenarios, provider surfaces, components, warning codes, source adapters, framework adapters, requirements, tags, and expected languages. |
...
| Normalize documentation layout | Done | `docs/guides/`, `docs/reference/`, `docs/internal/notes/`, `docs/internal/decisions/`, `docs/internal/reports/`, `docs/internal/process/`; hygiene and release scripts use new paths | Preserves content while moving dated/uppercase docs into categorized lowercase paths. |
...
| Milestone 1.5: Polyglot Toolchain Foundation | Complete for current scope | No | Decision record, manual type artifacts, parity matrix, machine-readable public API registry, generated public API docs, Go examples, explicit Go fixture check, generated taxonomy docs, generated schema-field docs, generated-doc drift checks, type-taxonomy parity checks, Python type-surface checks, hygiene checks, and CI workflow exist. | Full schema-derived contract model generation and more typed Go output surfaces remain later hardening items, not a current active lane. |
...
| Milestone 3: Source Adapter Layer | Complete for current scope | No | `llm-prices` current and historical feeds, LiteLLM, Portkey, OpenRouter models, models.dev, reviewed official/public-preview snapshots, source-cache envelopes, local JSON/YAML file loading, explicit refresh command, source capability warnings, user compact pricing, and Helicone prototype adapters exist. | Later source expansion moves to beta/V1 hardening. |
| Milestone 4: Provider Extractors V0 | Complete for current scope | No | OpenAI Responses, OpenAI Chat Completions, OpenAI organization usage completions buckets, OpenAI Embeddings, OpenAI organization usage embeddings buckets, OpenAI Audio Transcriptions, OpenAI organization usage audio transcription buckets, OpenAI Images, OpenAI organization usage image buckets, OpenAI organization usage audio speech buckets, OpenAI Vector Stores storage bytes, OpenAI organization usage code-interpreter sessions, Anthropic, OpenRouter, Groq, xAI Chat Completions and xAI Responses, Meta Model API Responses and Chat Completions, Mistral, DeepSeek, Azure OpenAI, Hugging Face, Cohere Chat and Rerank, Gemini/Vertex, Bedrock Converse, and Bedrock InvokeModel extractors exist for selected surfaces; selected final streaming usage cases are fixture-backed, including OpenAI Responses, OpenAI-compatible chat final-usage chunks, Anthropic Messages, and Gemini generateContent; OpenAI Conversations are documented as non-cost-bearing state resources whose costs attach to Responses. | Additional stream protocols, broader provider-specific storage/session, and deeper provider-specific feature fields move to beta/V1 hardening. |
...
- Added optional sanitized alpha smoke harness:
  ...
  - `scripts/check_alpha_smoke_contract.py`
  - `scripts/run_alpha_smoke_preflight.py`
  - `scripts/check_alpha_smoke_preflight.py`
  - `fixtures/source-files/alpha-smoke-samples.json`
- Added `schemas/alpha-smoke-report.schema.json` so sanitized alpha smoke and live preflight evidence have a durable schema-backed contract; the schema now has mode-specific constraints for aggregate smoke, single framework, and preflight report shapes, including required sample-price acknowledgement for smoke reports.
...
- Preflight mode reports credential and optional SDK dependency readiness plus sanitized next actions without emitting API-key values; in the current environment it reports only the local multi-provider discount scenario as ready.
...
- The aggregate Python smoke runner, LangChain smoke runner, and Vercel smoke runner validate their sanitized report shape before writing output so accidental forbidden fields fail closed.
- Aggregate framework child-script failures are captured as sanitized `needs_product_truth` results with error type only, so optional framework smoke failures do not abort the entire evidence report.
...
- Added `scripts/create_alpha_product_truth_entry.py` and `npm run alpha:truth-entry` so a sanitized live smoke finding can generate a schema-valid product-truth register entry from the report scenario, status, and next action without hand-authoring those fields.
...
- Added product-truth classification enforcement:
  - `fixtures/source-files/alpha-smoke-product-truth-register.json` records the current no-credential live findings and ties each skipped scenario to `docs/internal/reports/2026-05-26-alpha-smoke-live-no-credentials.md`;
  - `schemas/alpha-smoke-product-truth-register.schema.json` documents the machine-readable product-truth register contract;
  - `scripts/check_alpha_product_truth.py` validates a live smoke report against that register and fails if non-passing findings lack a product-truth artifact, use `classification: none`, or point at an incompatible artifact kind;
  - `npm run check:alpha-truth` and `npm test` now run the product-truth check.
- Captured that finding as `docs/internal/reports/2026-05-26-alpha-smoke-live-no-credentials.md`; this is product-truth documentation for the no-credential state, not completion of the live-provider-run gate.
- Added sanitized invoice/dashboard comparison mechanics:
  - `scripts/compare_invoice_dashboard.py`
  - `scripts/check_invoice_comparison_contract.py`
  - `scripts/check_invoice_comparison.py`
  - `schemas/invoice-dashboard-comparison-input.schema.json`
  - `schemas/invoice-dashboard-comparison.schema.json`
...
- `scripts/compare_invoice_dashboard.py` fails closed before writing output if the input is not marked safe to commit, contains private billing export data, uses an invalid evidence type, includes forbidden keys, or contains secret-like values.
- Sanitized invoice/dashboard comparison inputs now have a schema-backed contract, and the producer rejects unresolved provider paths, invalid field mappings, and exact/estimated mappings without a comparable RunCost value before writing output.
- Added `scripts/create_openai_costs_comparison_input.py`, `npm run compare:invoice:openai-costs-input`, and `fixtures/source-files/openai-costs-comparison-source.json` so a reviewed, sanitized OpenAI Costs API export can be reduced into the generic comparison-input contract before producing exact/estimated/unsupported rows.
- Added `scripts/run_openai_costs_invoice_comparison.py` and `npm run compare:invoice:openai-costs` so the OpenAI Costs path has a CI-safe sample mode and an `OPENAI_ADMIN_KEY`-gated live mode that strips provider identifiers and requires a supplied RunCost ledger for the same reviewed window.
- Added the OpenAI Costs invoice/dashboard comparison path to `scripts/run_alpha_smoke_preflight.py` so Milestone 8 readiness reports identify the required `OPENAI_ADMIN_KEY`, `openai_costs_start_time`, and `openai_costs_runcost_ledger` inputs without printing secret values, timestamps, or local paths.
...
- Added a Milestone 8 evidence bundle checker:
  - `scripts/check_alpha_evidence_bundle.py` validates required smoke scenarios, product-truth classifications, and invoice/dashboard comparison evidence as one reviewed set.
  - `npm run check:alpha-bundle` and `npm test` run the sample-mode bundle check.
  - The future strict command requires passing live-source evidence for OpenAI Responses, Anthropic caching, Vercel streaming, LangChain agent usage, and OpenRouter cost comparison, includes the multi-provider discount case in the same live review set, requires sample-price acknowledgement, and rejects sample invoice/dashboard comparisons as completion evidence.
- Added a Milestone 8 evidence bundle collector:
  - `scripts/collect_alpha_evidence_bundle.py` writes a sanitized manifest plus preflight, aggregate smoke, Vercel, LangChain, and invoice comparison artifacts into one evidence directory.
  - `scripts/check_alpha_evidence_collector.py`, `npm run alpha:bundle`, and CI prove the sample-mode collector path without credentials.
  - The collector accepts `--invoice-comparison` and `--require-real` for the future live evidence review, but sample mode remains explicitly non-completion evidence.
- Added a project completion gate register and checker:
  ...
  - `schemas/project-completion-gates.schema.json` documents the machine-readable gate register contract.
  - `scripts/check_project_completion_gates.py` validates the register, referenced evidence, evidence kinds, status-dependent gate rules, and schema/validator parity in normal CI.
...
- Advanced package publication readiness without publishing:
  ...
  - release readiness checks now reject stored PyPI/npm registry-token dependencies in the workflow, preserving the trusted-publishing/OIDC path while registry-side setup remains external.
  - release workflow now requires typed `publish_approval=publish-runcost` in addition to `publish=true`, preserving the current unpublished state until explicit approval.
  - added `schemas/trusted-publishing-verification.schema.json`, `fixtures/source-files/trusted-publishing-verification-template.json`, and `scripts/check_trusted_publishing_verification.py` so future PyPI/npm registry-side trusted-publisher setup can be recorded as sanitized evidence and checked with `--require-real` without storing registry tokens.
  - added `docs/internal/reports/2026-05-26-package-name-availability-check.md` after registry checks showed npm `runcost` is unclaimed or not publicly visible, but PyPI `runcost` is occupied through version `0.4`.
  - added `docs/internal/decisions/2026-05-27-python-distribution-name.md` and updated `pyproject.toml` so the Python distribution target is `runcost-ai`, while the import package and CLI remain `runcost`.
- Advanced polyglot generated-artifact hardening:
  ...
  - `scripts/generate_contract_docs.py` also generates `docs/generated/warning-coverage.md` from `schemas/taxonomy.json` and fixture warning metadata, currently showing all 19 warning codes fixture-backed;
  - `scripts/generate_contract_docs.py` also generates `docs/generated/beta-v1-caveats.md` from `fixtures/source-files/project-completion-gates.json` so public caveats stay aligned with strict gate status;
  - `scripts/generate_language_types.py` generates taxonomy-bearing Python, TypeScript, and Go type artifacts from `schemas/taxonomy.json`;
  - Python `types.py`, TypeScript `index.d.ts`, and Go component ordering now consume generated taxonomy artifacts instead of hand-maintained literal lists;
  ...
  - `scripts/check_type_taxonomy_parity.py` fails when generated taxonomy artifacts or public taxonomy-bearing type imports drift from `schemas/taxonomy.json`;
  - `npm run generate:contracts` refreshes the checked-in generated docs and generated language type artifacts.
...
- Advanced provider/feature breadth:
  ...
  - regenerated contract taxonomy docs and fixture coverage; fixture count is now 89.
  - added `fixtures/cohere-rerank-raw-billed-units.json` to prove provider-specific Cohere Rerank `meta.billed_units.search_units` extraction into `rerank_search_units` across Python, JavaScript/TypeScript, and Go;
  - added `extract_cohere_rerank_usage` and `extractCohereRerankUsage` package exports and dispatch support for `surface: "cohere.rerank"`;
  - regenerated generated support matrices and fixture coverage; fixture count is now 93.
  - added `fixtures/openai-chat-stream-final-usage-chunk.json` to prove OpenAI-compatible chat completion streaming final-usage chunk extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 94.
  - added `fixtures/openrouter-sdk-stream-provider-cost.json` to prove OpenRouter SDK/OpenAI-compatible streaming final-usage chunks can carry provider-reported cost comparison warnings across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 95.
  - added `fixtures/openai-audio-transcription-duration-usage.json` and `fixtures/openai-audio-transcription-token-usage.json` plus OpenAI audio transcription extractors across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 97.
  - added `fixtures/openai-images-token-usage.json` and `fixtures/openai-images-unit-usage.json` plus OpenAI Images extractors across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 99.
  - added `fixtures/openai-vector-store-storage-gb-days.json` plus OpenAI Vector Stores `usage_bytes` to explicit GB-day storage extractors across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 100.
  - added `fixtures/openai-usage-code-interpreter-sessions.json` plus OpenAI organization usage code-interpreter `num_sessions` extractors across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 101.
  - added `fixtures/openai-usage-images-buckets.json` plus OpenAI organization usage image bucket extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 102.
  - added `audio_generation_characters` with `character` units and `fixtures/openai-usage-audio-speeches-buckets.json` for OpenAI organization usage audio speech character extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 103.
  - added `fixtures/openai-usage-audio-transcriptions-buckets.json` plus OpenAI organization usage audio transcription seconds extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 104.
  - added `fixtures/openai-usage-embeddings-buckets.json` plus OpenAI organization usage embeddings input-token extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 105.
  - added `fixtures/openai-usage-completions-buckets.json` plus OpenAI organization usage completions text, cached-text, and audio token extraction across Python, JavaScript/TypeScript, and Go; regenerated generated support matrices and fixture coverage; fixture count is now 106.
  - added `examples/python_framework_adapters.py` and `examples/javascript_framework_adapters.mjs` as dependency-free framework examples derived from the alpha smoke paths; they exercise OpenAI Agents SDK, LangSmith, Vercel `streamText` finish/onFinish, and OpenRouter SDK cost-comparison behavior.
  ...
  - added opt-in `discount_not_applied` warnings for discount policies with `metadata.warn_if_unapplied: true`, reducing uncovered warning codes from 4 to 3.
  - added fixture-backed semantics for `unknown_provider`, `usage_field_ignored`, and `inclusive_usage_ambiguous` across Python, JavaScript, and Go; all 19 warning codes are now fixture-backed and fixture count is now 92.
- Advanced public-beta source-data process hardening:
  - added `docs/internal/process/2026-05-26-source-data-update-process.md` with owner, cadence, review checklist, and product-truth loop for source refreshes;
...
- Advanced V1 stabilization handoff:
  - added `schemas/v1-stabilization-checklist.schema.json`, `fixtures/source-files/v1-stabilization-checklist-template.json`, and `scripts/check_v1_stabilization_checklist.py` so the previous V1 checklist stub is now a strict release-candidate evidence gate rather than a prose placeholder.
- Attempted guarded no-publish release workflow dispatch:
  ...
  - captured the finding as `docs/internal/reports/2026-05-26-release-workflow-no-publish-blocked.md`;
...
- Reran the guarded no-publish workflow after the private-module fix; run
  `https://github.com/adamallcock/runcost/actions/runs/26467656634` passed,
  including `go get github.com/adamallcock/runcost/packages/go/ledger@v0.1.0`
  from a clean temporary module without a local `replace`; evidence is recorded
  in `docs/internal/reports/2026-05-26-go-tag-verification-0-1-0.md`.
- Added docs:
...
- Regenerated `docs/internal/reports/fixture-coverage.md`; fixture count is now 83 and `tool_pricing` tag coverage is now 4.
...
- Regenerated `docs/internal/reports/fixture-coverage.md`; fixture count is now 81 and framework-adapter scenario count is now 16.
...
- Added `docs/internal/decisions/polyglot-toolchain-decision.md` and `docs/internal/notes/api-parity-matrix.md`.
...
- Updated `README.md` and `docs/internal/project-plan.md` to reference the polyglot maintenance artifacts.
...
- Updated `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, and `scripts/check_project_hygiene.py` for the new correctness coverage.
...
- Added provider extractor mapping notes in `docs/internal/notes/provider-extractor-notes.md`.
...
- Updated `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, and hygiene checks for the new provider coverage.
...
- Updated `docs/internal/notes/provider-extractor-notes.md`, `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, and hygiene checks for the new OpenAI-compatible provider coverage.
...
- Updated `docs/internal/notes/provider-extractor-notes.md`, `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, Go comments, and hygiene checks for Cohere coverage.
...
- Updated `docs/internal/notes/provider-extractor-notes.md`, `docs/internal/notes/api-parity-matrix.md`, `scripts/check_project_hygiene.py`, and this tracker for the Vertex fixture evidence.
...
- Added `docs/internal/notes/framework-adapter-notes.md` for framework metadata mapping and limitations.
...
- Updated `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, and hygiene checks for framework adapter coverage.
...
- Updated `docs/internal/notes/framework-adapter-notes.md`, `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, Go API comments, and hygiene checks for helper coverage.
...
- Updated `docs/internal/notes/provider-extractor-notes.md`, `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, `scripts/check_project_hygiene.py`, and this tracker for multimodal coverage.
...
- Implemented OpenRouter models source adapters across Python, JavaScript/TypeScript, and Go:
  ...
  - `PriceCardsFromOpenRouterModels`
- Updated `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, TypeScript declarations, Python exports, Go API comments, fixture runners, hygiene checks, and this tracker for OpenRouter source coverage.
...
- Added fixture metadata and coverage slice:
  ...
  - Added generated coverage report `docs/internal/reports/fixture-coverage.md`.
...
- Added package publish-readiness slice:
  ...
  - Added `docs/internal/process/release-process.md` covering version policy, PyPI trusted publishing, npm trusted publishing/provenance, Go semantic version tags, pre-release checks, and rollback guidance.
...
- Regenerated `docs/internal/reports/fixture-coverage.md`.
...
- Added documented partial adapter paths in `docs/internal/notes/framework-adapter-notes.md` for:
  - Semantic Kernel.
  - Haystack.
  - AutoGen / AG2.
...
- Updated `docs/internal/notes/api-parity-matrix.md`, `docs/internal/project-plan.md`, `README.md`, and `docs/reference/warnings-and-limitations.md` so these paths are visible without overstating support.
...
- Moved existing docs into stable category paths:
  ...
  - `docs/internal/notes/` for API parity, framework adapter notes, and provider extractor notes.
  - `docs/internal/decisions/` for the polyglot toolchain decision record.
  - `docs/internal/reports/` for fixture coverage.
  - `docs/internal/process/` for release process docs.
...
- Verification after documentation layout stabilization slice:
  - `python3 scripts/check_project_hygiene.py` passed.
  - `python3 scripts/check_fixture_coverage.py --write-report` passed and regenerated coverage at `docs/internal/reports/fixture-coverage.md`.
  - `python3 scripts/check_fixture_coverage.py` passed.
  - `npm run check:release` passed.
  - `npm test` passed: 59 fixtures, fixture coverage, Go tests, and hygiene checks green.
  - `git diff --check` passed.
...
- Added a registry README policy and exact npm trusted-publisher setup notes to `docs/internal/process/release-process.md`.
...
4. Continue releasing through the guarded workflow: configure and verify
   ...
   only with explicit approval. Release `0.1.12` completed this path on
   2026-07-05.
...
- Current package surfaces inspected:
  ...
  - `docs/internal/product-requirements.md`
  - `README.md`
...
- JavaScript/TypeScript: first-class prototype ESM core, manual `index.d.ts` declarations, package metadata exists, package tarball install checks pass, publish allowlist exists, and `runcost@0.1.12` is published on npm with provenance.
...
- Source adapters for `llm-prices` current and historical feeds, LiteLLM, Portkey, OpenRouter models, models.dev, reviewed official/public-preview snapshots, source-cache envelopes, local JSON/YAML files, source capability warnings, user compact pricing, and Helicone model-registry data.
- Provider extractors for OpenAI Responses, OpenAI Chat Completions, OpenAI Embeddings, Anthropic Messages, OpenRouter chat completions, Groq, xAI Chat Completions and xAI Responses, Meta Model API Responses and Chat Completions, Mistral, DeepSeek, Azure OpenAI chat, Hugging Face Inference Providers chat, Cohere Chat, Gemini/Vertex generateContent, Bedrock Converse, and Bedrock InvokeModel with Anthropic Messages bodies, including selected final streaming usage envelopes for OpenAI Responses, Anthropic Messages, and Gemini generateContent.
...
1. Real invoice/dashboard comparison evidence: run a sanitized provider dashboard, invoice, usage export, or OpenAI Costs comparison against a matching RunCost ledger.
