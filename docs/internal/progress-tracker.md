...
- Real-life live SDK/API smoke tests are implemented as optional API-key-gated harnesses, and partial credentialed provider evidence now exists; OpenAI Responses, framework live SDK smoke, and invoice/dashboard evidence remain pending.
...
4. Continue releasing through the guarded workflow: configure and verify
   ...
   only with explicit approval. Release `0.1.11` completed this path on
   2026-07-02.
...
- JavaScript/TypeScript: first-class prototype ESM core, manual `index.d.ts` declarations, package metadata exists, package tarball install checks pass, publish allowlist exists, and `runcost@0.1.11` is published on npm with provenance.
