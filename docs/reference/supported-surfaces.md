...
- Pricing-period selection is fixture-backed for DeepSeek-style peak and
  ...
  current weekday-only UTC schedule. Direct DeepSeek auto-resolution retrieves
  it as an external checked cache source before generic catalogs; it is not a
  package-default catalog. When a provider response does not contain a usable
  timestamp, callers can set `context.priced_at` on normalized usage or pass an
  explicit `pricing_period` through their own usage ledger.
